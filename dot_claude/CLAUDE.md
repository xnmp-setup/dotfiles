...
- Fable tokens are very expensive. When the main model is Fable, delegate to subagents for less complex or well-scoped tasks (searching, reading, exploration, routine edits, test runs). Choose the appropriate model depending on the task difficulty. Prefer to use codex subagents for this if GPT-5.6 class models are available (use the `codex-subagent` skill). Intervene if a subagent goes off track or is missing relevant context. Reserve Fable for high-level reasoning and synthesis, or for high-risk high-complexity tasks, or when the cost of re-explaining context outweighs the savings. Do not use Fable subagents unless the task is particularly high risk or high complexity, or when it involves many cross-cutting concerns. 
...
- When making medium-high risk changes, or changes where there's a nontrivial error surface, use an independent adversarial reviewer to verify findings and spot potential issues. Do not bias the reviewer with your own preconceptions of what may be wrong - the point is to have a fresh set of eyes.
