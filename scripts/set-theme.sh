...
if [[ -f "$config" ]]; then
  ...
  if grep -q '^background-opacity[[:space:]]*=' "$config"; then
    ...
    echo "background-opacity = $ghostty_opacity" >> "$config"
  fi
...
#
# Optional solid desktop tokens preserve authored surfaces and borders.
# Themes without them retain the existing blends.
...
if [[ -f "$hypr_css" && -d "$hypr_dir" ]]; then
  ...
  if [[ -n "$h_bg" && -n "$h_accent" && -n "$h_text" ]]; then
    ...
    h_surface=$(desktop_color "$hypr_css" desktop-surface "$h_bg" "$h_text" 18)
    h_border=$(desktop_color "$hypr_css" desktop-border "$h_bg" "$h_text_dim" 30)
...
if [[ ! -f "$greeter_css_source" ]]; then
  ...
  if [[ -z "$g_bg" || -z "$g_accent" || -z "$g_text" ]]; then
    ...
    # Share Hyprland's authored desktop surfaces, with legacy mix fallbacks.
    g_surface=$(desktop_color "$greeter_css_source" desktop-surface "$g_bg" "$g_text" 18)
    g_border=$(desktop_color "$greeter_css_source" desktop-border "$g_bg" "$g_text_dim" 30)
    g_opacity=0.82
    g_shadow=0.45
    if [[ "$theme_mode" == light ]]; then
      g_opacity=0.97
...
/* Light surfaces admit a faint wallpaper tint; dark surfaces admit more. */
frame.background {
    background-color: rgba($g_bg_rgb, $g_opacity);
    border: 1px solid #$g_border;
    border-radius: 14px;
    box-shadow: 0 8px 32px rgba(0, 0, 0, $g_shadow);
}
